...
describe('move', () => {
  ...
  it(`throws error if given list's folder ID is empty`, () => {
    const list = [{ id: '', name: 'Folder 1', files: [] }];

    expect(() => move(list, '2', '4')).toThrow('Folder ID cannot be empty');
  });

  it(`throws error if given list's folders' file ID is empty`, () => {
    const list = [
      {
        id: '1',
        name: 'Folder 1',
    ...
    expect(() => move(list, '7', '1')).toThrow('File ID cannot be empty');
  });
});
